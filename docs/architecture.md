...
Static map of routes, API endpoints, components, tests, integrations, and config
hints. In a Git worktree, the index is built from tracked files plus untracked,
non-ignored files; Git ignores and Preflight Scout's unconditional credential/build
artifact exclusions are applied before the file limit. The LLM-facing view uses
repo-relative paths and redacts the absolute checkout root and detected secrets.
